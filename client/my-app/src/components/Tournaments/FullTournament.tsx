...
const FullTournament = () => {
    ...
    //     instance.patch(`/tournaments/${id}`, followed)
    //         .then(({data}) => {
    //             return data
    //         })
    // }, [Follow])
    ...
    return (
        <div className="App">
            ...
            <div>
                ...
                <h3> {data.about}</h3>
                {data.followed
                    ? <button onClick={() => setFollow(false)}>Покинуть турнир</button>
                    : <button onClick={() => setFollow(true)}>Принять участие в турнире</button>
                }
                {data.followed ? <span>Вы отслеживаете этот турнир</span>
                    : <span>Вы не отслеживаете этот турнир</span>}
